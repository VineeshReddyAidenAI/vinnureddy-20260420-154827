...
export default function CreateAccountWithNric24ErrorPasswordNotMatched() {
  const navigate = useNavigate()
  const [password, setPassword] = useState('abcdef')
  const [confirmPassword, setConfirmPassword] = useState('arC01#$12?_')
  const [showPassword, setShowPassword] = useState(false)
  const [showConfirmPassword, setShowConfirmPassword] = useState(false)
  const [error, setError] = useState('Password do not match, try again.')
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (error && e.target.value === confirmPassword) {
      setError('')
    }
  }
